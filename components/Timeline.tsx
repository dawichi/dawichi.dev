...
const Timeline = () => {
	...
	const Timeline = ({timelines}) => timelines.map((timeline:any) => (
		<div>
				...
				<ul className='border-l-4 border-teal-500 bg-white/50 dark:bg-zinc-800 shadow rounded-r-lg p-4 md:translate-x-[6rem]'>
					{ timeline.list.map((element:any, idx:number) => (
						<li key={idx} className='p-4 border-b-2 relative last:border-0'>
							...
							{ element.months
								? <p><a className='text-teal-500' href={'https://' + element.url} target="_blank">{element.title}</a> - {element.months} months</p>
								: <p>{element.title}</p>
							}
						</li>
					))}
	...
	return (
		<div className='container mx-auto mt-40 p-5'>
			...
			<div className='grid lg:grid-cols-2 mt-5'>
				...
				<div><img src="/assets/svg/study.svg" className='opacity-90 mx-auto m-5 w-3/4' alt=""/></div>
			</div>
		</div>
	)
}
